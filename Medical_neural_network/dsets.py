...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob(data_path + 'luna/subset0/*.mhd')
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    diameter_dict = {}
    with open(data_path + 'luna/annotations.csv', "r") as f:
    ...
    candidateInfo_list = []
    with open(data_path + 'luna/candidates.csv', "r") as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
...
class Ct:
    def __init__(self, series_uid):
        mhd_path = glob.glob(
            data_path + 'luna/subset*/{}.mhd'.format(series_uid)
        )[0]
